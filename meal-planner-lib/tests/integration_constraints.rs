...
use meal_planner_lib::database_access::{DataBaseTypes, Database, DbSearchCriteria, get_db};

async fn mock_db() -> Box<dyn Database> {
    get_db(DataBaseTypes::Mock)
        .await
        .expect("mock database should be available")
}
...
fn test_user_can_build_day_plan_from_mock_db() {
    block_on(async {
        ...
        let results = db.get_products_matching_criteria(&criteria).await;
...
fn test_user_receives_infeasible_error_from_mock_db() {
    block_on(async {
        let db = mock_db().await;
        let results = db
            .get_products_matching_criteria(&[DbSearchCriteria::ById("Banana".to_string())])
            .await;
        assert_eq!(results.len(), 1);
        let banana = results
